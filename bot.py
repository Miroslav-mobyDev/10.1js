...
BOT_TOKEN = "8016682643:AAF6rnAmFgkjWrtYJRk4Tg8uv_Jgzqk3iKg"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "send_check":
        ...
        # Создаем временный Update объект для help_command
        temp_update = Update(0)
        temp_update.message = query.message
        await help_command(temp_update, context)
        return WAITING_FOR_CHECK
    elif query.data == "restart":
        # Создаем временный Update объект для start
        temp_update = Update(0)
        temp_update.message = query.message
        return await start(temp_update, context)
...
async def admin_approval_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        try:
            ...
            # Здесь нужно обновить состояние через ConversationHandler
            # Для этого сохраняем информацию о том, что чек одобрен
...
async def receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, что чек был одобрен
    if user_id not in pending_approvals or not pending_approvals[user_id].get("approved"):
        await update.message.reply_text(
            "❌ Сначала необходимо отправить и получить подтверждение чека.\n"
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_approvals:
        if pending_approvals[user_id].get("approved"):
            await update.message.reply_text(
                "✅ **Статус:** Чек подтвержден!\n\n"
                "Пожалуйста, введите ваше имя для продолжения регистрации."
            )
        else:
            await update.message.reply_text(
